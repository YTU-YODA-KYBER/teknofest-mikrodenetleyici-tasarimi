...
assign we = axi_data_bram_awvalid && axi_data_bram_wvalid && axi_data_bram_awready && axi_data_bram_wready;

assign waddr = axi_data_bram_awaddr[ADDR_WIDTH+1:2];
assign wdata = axi_data_bram_wdata;

assign axi_data_bram_bresp = 0;

always_ff @(posedge clk_i or negedge rst_n) begin
    if (!rst_n) begin
        axi_data_bram_bvalid  <= 0;
        axi_data_bram_awready <= 1;
        axi_data_bram_wready  <= 1;
    end
    // ÖNCELİK 1: B handshake — response kabul edildi, sıfırla
    else if (axi_data_bram_bvalid && axi_data_bram_bready) begin
        axi_data_bram_bvalid  <= 0;
        axi_data_bram_awready <= 1;
        axi_data_bram_wready  <= 1;
    end
    // ÖNCELİK 2: AW+W handshake — slave hazırken (awready=1, wready=1) kabul et
    else if (axi_data_bram_awvalid && axi_data_bram_awready &&
             axi_data_bram_wvalid  && axi_data_bram_wready) begin
        axi_data_bram_bvalid  <= 1;
        axi_data_bram_awready <= 0;
        axi_data_bram_wready  <= 0;
    end
end



assign raddr = axi_data_bram_araddr[ADDR_WIDTH+1:2]; 

always_ff @(posedge clk_i or negedge rst_n) begin
    if (!rst_n)
        rdata_latch <= '0;
    else if (axi_data_bram_arvalid && axi_data_bram_arready)
        rdata_latch <= rdata;
end
...
always_ff @(posedge clk_i or negedge rst_n) begin
    if (!rst_n) begin
        axi_data_bram_rvalid <= 0;
        axi_data_bram_arready <= 1;
        axi_data_bram_rresp <= 0;
    end
    else if(axi_data_bram_arvalid && axi_data_bram_arready)begin
        axi_data_bram_rvalid <= 1;
        axi_data_bram_arready <= 0;
    end
    else if (axi_data_bram_rvalid && axi_data_bram_rready)begin
        axi_data_bram_rvalid <= 0;
        axi_data_bram_arready <= 1;
    end
end
