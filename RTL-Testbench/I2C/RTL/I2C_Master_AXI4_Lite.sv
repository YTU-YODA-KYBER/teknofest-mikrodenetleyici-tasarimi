...
(
    ...
    wire I2C_SDA_out;  // I2C_SDA'ya sürülecek değer (0 veya high-Z)
    wire I2C_SDA_oe;   // I2C_SDA'nın sürülüp sürülmeyeceğini kontrol eden sinyal (1=sür, 0=serbest bırak)
    wire I2C_SDA_in;   // I2C_SDA'dan okunan değer (0 veya 1)

    // =====================================================
    // Bidirectional I/O Buffers (Xilinx primitive)
    // =====================================================
    IOBUF iobuf_i2c_sda (
        .I (I2C_SDA_out),   // sürülecek değer
        .T (~I2C_SDA_oe),   // tristate enable (active LOW: 0=sür, 1=bırak)
        .O (I2C_SDA_in),    // okunan değer
        .IO(I2C_SDA)        // fiziksel pin (top_module inout port)
    );
    ...
    assign I2C_SDA_out = 1'b0;         // I2C open-drain: sadece 0 sürer
    assign I2C_SDA_oe  = ~sda_out_val; // sda_out_val=0 iken sür, =1 iken bırak
                ...
                if (freq_div_cnt == 29 && freq_div_en)begin
                    if(restart)begin
                        ...
                        case(current_state)
                            ...
                            ack: begin
                                if(I2C_SCL && !I2C_SDA_in) current_state <= stop_and_clear;
                            end
